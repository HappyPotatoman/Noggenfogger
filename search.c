    ...
    Depth depth, bool cutNode, const int NT)
{
  ...
  if (PvNode && !ttMove)
    depth -= 2 + 2 * (ss->ttHit && tte_depth(tte) >= depth);
...
moves_loop: // When in check search starts from here
  ...
  while ((move = next_move(pos, moveCountPruning))) {
        ...
        && (!PvNode || ss->ply > 1 || pos->threadIdx % 4 != 3))
    {
      ...
      if (!PvNode && pos->rootDepth > 10 && pos->bestMoveChanges <= 2)
        r++;
      ...
      Depth d = clamp(newDepth - r, 1, newDepth + (r < -1 && (moveCount <= 5 || (depth > 5 && PvNode))));
