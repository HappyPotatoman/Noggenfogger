...
//      && !Skill(option_value(OPT_SKILL_LEVEL)).enabled()
  ...
  if (bestThread->rootMoves->move[0].pvSize > 2 || extract_cont_from_tt(&bestThread->rootMoves->move[0], pos)) 
  ...
  */
  ...
  if (!inCheck && (!bestMove || !is_capture(pos, bestMove))
      ...
      && !(!bestMove && bestValue >= ss->staticEval))
      {
        ...
        clamp_correction_histories(&nonPawnCorrectionHistory[WHITE][us][ss->nonPawnKey[WHITE] & (PAWN_CORRECTION_HISTORY_SIZE - 1)], bonus * 165 / 128);
        clamp_correction_histories(&nonPawnCorrectionHistory[BLACK][us][ss->nonPawnKey[BLACK] & (PAWN_CORRECTION_HISTORY_SIZE - 1)], bonus * 165 / 128);
      }
